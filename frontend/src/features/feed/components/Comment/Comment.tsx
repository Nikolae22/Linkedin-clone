    ...
    <div key={comment.id} className={classes.root}>
      {!editing ? (
        <>
          <div className={classes.header}>
            ...
            >
              ...
              <div className={classes.date}>
                {timeAgo(new Date(comment.updateDate || comment.creationDate))}
                {comment.updateDate ? " . Edited " : ""}
              </div>
            </button>
            {comment.author.id == user?.id && (
              <button
                className={`${classes.action} ${showAction ? classes.active : ""}`}
                onClick={() => setShowActions(!showAction)}
              >
          ...
          <div className={classes.contet}>{comment.content}</div>
        </>
      ) : (
        <form
          onSubmit={async (e) => {
